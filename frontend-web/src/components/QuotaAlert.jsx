...
export default function QuotaAlert({ quota }) {
  ...
  if (percentage >= 95) {
    alertLevel = 'error';
    message = t('quotaAlertCritical') || `Stockage critique ! Vous avez utilisé ${percentage.toFixed(1)}% de votre espace. Mettez à niveau maintenant pour éviter d'être bloqué.`;
    icon = '🚨';
  } else if (percentage >= 90) {
    alertLevel = 'warning';
    message = t('quotaAlertHigh') || `Attention ! Votre stockage atteint ${percentage.toFixed(1)}%. Pensez à mettre à niveau votre plan.`;
    icon = '⚠️';
  } else if (percentage >= 80) {
    alertLevel = 'info';
    message = t('quotaAlertMedium') || `Votre stockage atteint ${percentage.toFixed(1)}%. Vous pouvez mettre à niveau votre plan pour plus d'espace.`;
    icon = 'ℹ️';
  }
