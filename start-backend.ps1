...
try {
    ...
    if ($mongoCheck.TcpTestSucceeded) {
        ...
        if (Test-Path ".env") {
            ...
            if (-not ($envContent -match "MONGO_URI=mongodb://localhost:27017/supfile")) {
                ...
                if ($envContent -match "MONGO_URI=.*") {
                    $envContent = $envContent -replace "MONGO_URI=.*", "MONGO_URI=mongodb://localhost:27017/supfile"
                } else {
                    $envContent += "`nMONGO_URI=mongodb://localhost:27017/supfile`n"
                }
            ...
            $envLines = @(
                ...
                "",
                "# JWT Secrets",
                "JWT_SECRET=supfile_dev_secret_key_change_in_production_min_32_chars_long",
                "JWT_REFRESH_SECRET=supfile_dev_refresh_secret_key_change_in_production_min_32_chars_long",
                "",
                "# Upload",
